...
class UNetInferenceAgent:
    ...
    def single_volume_inference_unpadded(self, volume):
        ...
        for idx in range(volume.shape[0]):            
            ...
            outputs[idx,:,:] = torch.argmax(pred, dim=0)
    ...
    def single_volume_inference(self, volume):
        ...
        for idx in range(volume.shape[0]):
            slc = volume[idx]
            slc = np.reshape(slc, (1, 1, self.patch_size, self.patch_size))
            slc = torch.from_numpy(slc).to(self.device, dtype=torch.float)
            pred = self.model(slc)
            pred = np.squeeze(pred.cpu().detach())
